Public Class Modelo
    Public Function GetById(pId As Int16) As EL.Modelo
        ...
        con.EjecutarConsulta("select * from dbo.Autos_Modelos where id_modelo = " & pId)
        con.adp.Fill(datatable)

        modelo.Id = datatable.Rows(0).ItemArray(1).ToString()
        modelo.Descripcion = datatable.Rows(0).ItemArray(2).ToString()
        modelo.Marca = New Marca().GetById(datatable.Rows(0).ItemArray(0).ToString())
        Return modelo
    End Function
    Public Function Listar() As List(Of EL.Modelo)
        ...
        con.EjecutarConsulta("Select id_marca From dbo.Autos_Modelos")
        con.adp.Fill(datatable)
    ...
    Public Function Listar(pIdMarca As Int16) As List(Of EL.Modelo)
        ...
        Dim datatable As New DataTable()
        Dim modelos As New List(Of EL.Modelo)
        con.EjecutarConsulta("Select id_modelo From dbo.Autos_Modelos where id_marca = " & pIdMarca)
        con.adp.Fill(datatable)
        For index = 0 To datatable.Rows.Count - 1
            modelos.Add(New Modelo().GetById(datatable.Rows(index).ItemArray(0).ToString()))
        Next
